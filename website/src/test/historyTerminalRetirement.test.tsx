...
import { __resetHistoryTerminalCleanupForTests, deleteHistoryWithTerminals, retryHistoryTerminalCleanup, reviveRetiredTerminals } from '../lib/historyTerminalCleanup'
...
describe('confirmed history deletion retires only that terminal generation', () => {
  ...
  // The existence check that gates revival is a request; its failure is an
  // error (errors-use-error-notice), so it lands in the cleanup notice with a
  // retry instead of being swallowed. A 404 is the designed outcome — the
  // chat is gone — and stays silent.
  it('surfaces a failed revival check in the cleanup notice and revives on retry', async () => {
    await addTab(undefined, 'A')
    await retireTerminalSession(await prepareTerminalRetirement('A'))
    const detail = vi.spyOn(api, 'chatSlotDetail').mockRejectedValueOnce(new ApiError(503, 'unavailable'))
    const store = historyStore(); renderWithProviders(<HistoryTerminalCleanupNotice />, { store })
    await act(async () => { await reviveRetiredTerminals('A') })
    expect(getBottomTerminalSnapshot('A').retired).toBe(true)
    const notice = screen.getByTestId('history-terminal-cleanup-error')
    expect(notice).toHaveTextContent('Could not check whether chat “A” still exists')
    detail.mockResolvedValueOnce({ messages: [], key: 'A', state: 'idle' })
    fireEvent.click(screen.getByRole('button', { name: 'Retry', exact: true }))
    await waitFor(() => expect(screen.queryByTestId('history-terminal-cleanup-error')).not.toBeInTheDocument())
    expect(detail).toHaveBeenCalledTimes(2)
    expect(getBottomTerminalSnapshot('A').retired).toBe(false)
  })
  it('keeps a deleted chat retired and silent when the revival check answers 404', async () => {
    await addTab(undefined, 'A')
    await retireTerminalSession(await prepareTerminalRetirement('A'))
    vi.spyOn(api, 'chatSlotDetail').mockRejectedValueOnce(new ApiError(404, 'gone'))
    const store = historyStore(); renderWithProviders(<HistoryTerminalCleanupNotice />, { store })
    await act(async () => { await reviveRetiredTerminals('A') })
    expect(getBottomTerminalSnapshot('A').retired).toBe(true)
    expect(screen.queryByTestId('history-terminal-cleanup-error')).not.toBeInTheDocument()
  })
  it('leaves another phase\'s failure notice in place instead of replacing it', async () => {
    for (let i = 0; i < 2; i++) await addTab(undefined, 'A')
    vi.mocked(api.deleteSession).mockImplementationOnce(async () => {
      vi.spyOn(persistence, 'withTerminalStateLock').mockRejectedValueOnce(new Error('quota'))
      return { ok: true }
    })
    ...
    await act(async () => { await store.dispatch(deleteHistorySession('dashboard_A')).unwrap() })
    expect(screen.getByTestId('history-terminal-cleanup-error')).toHaveTextContent('terminal cleanup failed')
    const detail = vi.spyOn(api, 'chatSlotDetail').mockRejectedValue(new ApiError(503, 'unavailable'))
    await act(async () => { await reviveRetiredTerminals('A') })
    expect(detail).not.toHaveBeenCalled()
    expect(screen.getAllByTestId('history-terminal-cleanup-error')).toHaveLength(1)
    expect(screen.getByTestId('history-terminal-cleanup-error')).toHaveTextContent('terminal cleanup failed')
  })
