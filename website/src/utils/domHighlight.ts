 ...
 * `::highlight(mc-chat-current)`; only color, background-color, text-decoration
 * and text-shadow are honored there.
 *
 ...
 *
 * Browsers without the API paint nothing; nothing here can throw or mutate.
 */
...
function highlightApi(): HighlightApi | null {
  ...
  // Re-register on every use: cheap, and it heals a registry another surface
  // cleared wholesale (a `CSS.highlights.clear()` elsewhere would otherwise
  // silently blank the transcript's matches for the rest of the page's life).
  if (CSS.highlights.get(SEARCH_HL_MATCH) !== shared.match) CSS.highlights.set(SEARCH_HL_MATCH, shared.match)
  if (CSS.highlights.get(SEARCH_HL_CURRENT) !== shared.current) CSS.highlights.set(SEARCH_HL_CURRENT, shared.current)
  return shared
}

/**
 * Paint every occurrence of `term` inside `el`.
