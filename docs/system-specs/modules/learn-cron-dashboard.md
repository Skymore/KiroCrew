...
**Workspace fullscreen and title-row ownership**: the chat route provides one
`WorkspaceFullscreenContext` across the portaled SidePanel. Fullscreen changes the
existing panel host's dimensions; the SidePanel and every body-owning tab stay mounted,
so editor drafts, browser frames, terminal scrollback and tab order survive both
directions. A `MarkdownPanel` hosted under that context omits its file-only fullscreen
action; standalone previews outside the context retain it. Escape is handled after
...
search, or leaving chat clears fullscreen.

The Sessions sidebar, main chat and SidePanel title actions share 40px rows, 28px
action cells, 14px icons and a 6px gap. A closed SidePanel leaves the main
row in the order Pop out → Split view → Bottom panel → Side panel. An open SidePanel
keeps its fused browser tabs and half-card frame and owns the trailing order Fullscreen
→ Bottom panel → Side panel. Component ownership stays split between fullscreen and
the two panel toggles, but the toolbar draws no divider and keeps one 6px visual
rhythm. The browser tabs are 32px chips fused to the body seam, 8px below the strip's
top edge, so their center sits 4px under the 28px action cells; the `+` trigger follows
the chips' center (Chrome's new-tab placement) while the trailing action group keeps the
row's center. The Side panel toggle
has the same bounding box before and after opening; the dock menu stays separate. The
Bottom-panel toggle ("Toggle terminal" as both tooltip and accessible name; the Side-panel
toggle is "Toggle side panel", state via `aria-pressed`; while the terminal is popped out the
control is named "Focus popped-out window", since the click focuses that window instead of
toggling) is a second entry to the docked
terminal next to the Side-panel toggle, kept alongside the nav rail's Terminal row and the chord: the two panels share
the workspace edge, and removing the nav-rail row would take a known entry away from
existing users.
