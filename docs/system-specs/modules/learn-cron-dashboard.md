...
row in the order Pop out → Split view → Bottom panel → Side panel, with the first
pair forming the session-window region and the second pair forming the
...
the chips' center (Chrome's new-tab placement) while the trailing action regions keep
the row's center. The Side panel toggle has the same bounding box before and after
opening because the separator occupies only the visibility region's leading edge. The
...
toggling) is the pointer entry to the docked
terminal, sitting next to the Side-panel toggle because the two panels share the workspace
edge; the rebindable `terminal` chord is the keyboard entry. The nav rail carries no
Terminal row. `PanelToggles` renders the terminal control only while
...
leave the panel with no toggle.
